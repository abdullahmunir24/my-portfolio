...
const Skills = ({ skills }) => {
  ...
  const webDevelopment = skills.filter((skill) =>
    ['HTML', 'CSS', 'React.js', 'Node.js', 'Firebase', 'Bootstrap', 'TailWind CSS'].includes(skill.name)
  );

  const dataAnalysis = skills.filter((skill) =>
    ['Pandas', 'NumPy', 'Seaborn', 'Matplotlib', 'SQL', 'MS Excel', 'Tableau'].includes(
      skill.name
    )
  );

  const appDevelopment = skills.filter((skill) => ['Swift', 'Firebase'].includes(skill.name));

  const toolsAndTechnologies = skills.filter((skill) =>
    ['GitHub', 'VS Code', 'Eclipse'].includes(skill.name)
  );

  const certifications = skills.filter((skill) => [ "MTA 98-381: Passed the exam with a score of 91/100"].includes(skill.name));
  ...
  return (
    <Element name="skills">
      <div className="skills-carousel">
        ...
        <Carousel ref={carouselRef} breakPoints={breakPoints} enableAutoPlay={false}>
          <div>
...
<img src={skill.image} style={{ width: '220px', height: '220px'}} />
